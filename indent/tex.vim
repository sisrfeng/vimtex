...
fun! VimtexIndentExpr() abort
    return VimtexIndent(v:lnum)
endf

"}}}
fun! VimtexIndent(lnum) abort
  let s:sw = shiftwidth()
...
fun! s:get_prev_lnum(lnum) abort
  let l:lnum = a:lnum
...
fun! s:clean_line(line) abort
  return substitute(a:line, '\s*\\\@<!%.*', '', '')
endf


fun! s:is_verbatim(line, lnum) abort
...
let s:indent_amps = {}
fun! s:indent_amps.check(lnum, cline, plnum, pline) abort dict
  let self.finished = 0
  let self.amp_ind = -1
  let self.init_ind = -1
...
fun! s:indent_amps.parse_context(lnum, line) abort dict
  let l:depth = 1
...
fun! s:indent_envs(line, prev_line) abort
  let l:ind = 0
...
fun! s:indent_items(line, lnum, prev_line, prev_lnum) abort
...
fun! s:indent_delims(line, lnum, prev_line, prev_lnum) abort
...
fun! s:indent_conditionals(line, lnum, prev_line, prev_lnum) abort
  if !exists('s:re_cond')
...
fun! s:indent_tikz(lnum, prev) abort
