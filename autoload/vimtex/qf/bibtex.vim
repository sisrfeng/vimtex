function! vimtex#qf#bibtex#addqflist(blg) abort
...
function! s:qf.addqflist(blg) abort
    let self.file = a:blg
...
function! s:qf.get_db_files() abort
...
function! s:qf.get_key_loc(key) abort
...
function! s:type_syn_error.fix(ctx, entry) abort
...
let s:type_empty = {
            ...
            \}
function! s:type_empty.fix(ctx, entry) abort
...
let s:type_style_file_defined = {
            ...
            \}
function! s:type_style_file_defined.fix(ctx, entry) abort
    let l:matches = matchlist(a:entry.text, self.re)
    if empty(l:matches) | return 0 | endif
...
function! s:type_no_bibstyle.fix(ctx, entry) abort
