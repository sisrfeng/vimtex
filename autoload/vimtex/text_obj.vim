...
fun! vimtex#text_obj#init_buffer() abort " {{{1
    ...
    for [l:map, l:name, l:opt] in [
        ...
        exe     printf('xnoremap <silent><buffer> <plug>(vimtex-i%s) :<c-u>call vimtex#text_obj#%s(1, 1%s)<cr>', l:map, l:name, l:optional)
        exe     printf('xnoremap <silent><buffer> <plug>(vimtex-a%s) :<c-u>call vimtex#text_obj#%s(0, 1%s)<cr>', l:map, l:name, l:optional)
        exe     printf('onoremap <silent><buffer> <plug>(vimtex-i%s) :<c-u>call vimtex#text_obj#%s(1, 0%s)<cr>', l:map, l:name, l:optional)
        exe     printf('onoremap <silent><buffer> <plug>(vimtex-a%s) :<c-u>call vimtex#text_obj#%s(0, 0%s)<cr>', l:map, l:name, l:optional)
    endfor
endf

" }}}1

fun! vimtex#text_obj#commands(is_inner, mode) abort " {{{1
    let l:obj = {}
...
" }}}1
fun! vimtex#text_obj#delimited(is_inner, mode, type) abort " {{{1
    let l:object = {}
    let l:prev_object = {}
    let l:pos_save = vimtex#pos#get_cursor()
    ...
    " Apply selection
    exe     'normal!' l:object.select_mode
    call vimtex#pos#set_cursor(l:object.pos_start)
    normal! o
...
" }}}1
fun! vimtex#text_obj#sections(is_inner, mode) abort " {{{1
    let l:pos_save = vimtex#pos#get_cursor()
...
" }}}1
fun! vimtex#text_obj#items(is_inner, mode) abort " {{{1
    let l:pos_save = vimtex#pos#get_cursor()
    ...
    " Apply selection
    if a:is_inner
        exe     'normal!' (v:operator ==# ':') ? visualmode() : 'v'
    el
        normal! V
    en
    call vimtex#pos#set_cursor(l:pos_start)
...
fun! s:get_sel_delimited_visual(is_inner, type, startpos) abort " {{{1
    if a:is_inner
...
" }}}1
fun! s:get_sel_delimited(open, close, is_inner) abort " {{{1
...
" }}}1
fun! s:get_sel_sections(is_inner, type) abort " {{{1
    let l:pos_save = vimtex#pos#get_cursor()
...
" }}}1
fun! s:get_sel_items(is_inner) abort " {{{1
...
fun! s:get_surrounding(type) abort " {{{1
    if a:type ==# 'delims'
