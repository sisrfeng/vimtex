...
fun! s:init_buffer() abort " {{{1
    ...
    " filetype "tex" or "bib".
    " A lot of settings are shared, but some are  different.
    ...
    aug  vimtex_buffers
        ...
        au BufUnload   <buffer> call s:buffer_deleted('unload')
        au BufWipeout  <buffer> call s:buffer_deleted('wipe')
    aug  END
    ...
    if &filetype ==# 'tex'
        ...
        setl     iskeyword+=:
        setl     includeexpr=vimtex#include#expr()
